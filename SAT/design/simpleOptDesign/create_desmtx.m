...
trials_per_cond=[10 10];     % number of trials in each condition (length must much nconds)
maxRep=2;                       % maximum number of repeat trials from same condition
trial_length=15;                 % trial length (in seconds)
ntrials=sum(trials_per_cond); % computes total number of trials
minISI=10;               % mininimum interstimulus interval (in seconds)
maxISI=16;               % maximum interstimulus interval (in seconds)
meanISI=12;              % desired mean interstimulus interval (in seconds)
restBegin=18;            % amount of rest to add in beginning of scan (in seconds)
restEnd=12;             % amount of rest to add at end of scan (in seconds)
scan_length=ceil((restBegin + restEnd + ntrials*(meanISI+trial_length))/TR);  % computes total scan length (in TRs)
TReff=TR/36;            % computes effective TR
