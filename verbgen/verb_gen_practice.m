function verb_gen_practice(TESTING)
  ...
  for iBlock = 1:3    % each block
    ...
    for iTrial = 1:nTrialPerBlock % start of trial
      ...
      if ~flag_key
        switch iBlockType
          case {1,2}
            % 03/25/2013 JMT Replace g/generate with D/Denken
            stimsize = fontscale * length('D');
            sxmid   = round(sx/2) - stimsize;
            Screen(w, 'DrawText','D', sxmid, symid, 255);
            Screen('Flip',w)
          case 3
            % 03/25/2013 JMT Replace r/read with L/Lesen
