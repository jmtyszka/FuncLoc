...
trials_per_cond=[6 6 6 6];     % number of trials in each condition (length must much nconds)
maxRep=2;                       % maximum number of repeat trials from same condition
trial_length=18;                 % trial length (in seconds)
% breakdown: 
% cueDur=1.25;
% stimDur=2.5;
% IEI=.15;
...
minISI=0;               % mininimum interstimulus interval (in seconds)
maxISI=0;               % maximum interstimulus interval (in seconds)
meanISI=0;              % desired mean interstimulus interval (in seconds)
restBegin=5;            % amount of rest to add in beginning of scan (in seconds)
restEnd=5;             % amount of rest to add at end of scan (in seconds)
scan_length=ceil((restBegin + restEnd + ntrials*(meanISI+trial_length))/TR);  % computes total scan length (in TRs)
TReff=TR/47;            % computes effective TR
